...
//            setBackgroundResource(R.color.cl_snappingstepper_button_press);
    ...
    private void updateUI() {
        ...
        if (mode == Mode.AUTO)//AUTO模式，写数值到滑动条上
            tvStepperContent.setText(String.valueOf(value));
        if (listener != null && b)  // 0503想改成value小于minValue 不触发listener；
     ...
     * @param b
     */
    public void setLeftButtonEnable(boolean b) {
        ivStepperMinus.setEnabled(b);
    }

    public void setRightButtonEnable(boolean b) {
        ivStepperPlus.setEnabled(b);
    }
}
