...
import static com.emiancang.emiancang.uitl.SharedPrefsUtil.getValue;

/**
 * Created by 22310 on 2016/11/7.
 */

public class EnterpriseNameActivity extends TitleActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!TextUtils.isEmpty(custName))
            ...
            registerPwdEt.setHint("请输入企业名称");
    }

    private void initHead() {
        Util.setStatusBarColor(getActivity(), R.color.green);
        getHeadBar().setBackground(getResources().getColor(R.color.green));
        getHeadBar().setCenterTitle("企业名称", getResources().getColor(R.color.white));
        ...
        getHeadBar().setRightOnClickListner(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(!TextUtils.isEmpty(name)){
                    EventBus.getDefault().post(new EnterpriseAccountTransfer(name,EnterpriseAccountTransfer.ENTERPRISENAME));
                    finish();
                }else{
                    showToast("企业名称不能为空");
                }
