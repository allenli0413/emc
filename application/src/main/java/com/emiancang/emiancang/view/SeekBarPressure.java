...
public class SeekBarPressure extends View {
    ...
    private double defaultMIN = 0;      //默认最小值
    private double defaultMAX = 100;    //默认最大值
...
//        this.setBackgroundColor(Color.BLACK);
        ...
        mScollBarHeight = (int) (notScrollBarBg.getIntrinsicHeight() * 1.5);

        mThumbWidth = (int) (mThumbLow.getIntrinsicWidth() * 1.5);
        mThumbHeight = (int) (mThumbLow.getIntrinsicHeight() * 1.5);
...
//        int height = measureHeight(heightMeasureSpec);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        double cha = defaultMAX - defaultMIN;
        double low = progressLow / 100 * cha + defaultMIN;
        double high = progressHigh / 100 * cha + defaultMIN;
//            Log.d(TAG, "onDraw-->mOffsetLow: " + mOffsetLow + "  mOffsetHigh: " + mOffsetHigh   + "  progressLow: " + progressLow + "  progressHigh: " + progressHigh);
        canvas.drawText((int) low + "", (int)mOffsetLow - 2 - 2, 15, text_Paint);
        canvas.drawText((int) high + "", (int)mOffsetHigh - 2, 15, text_Paint);
...
//            Log.d("ACTION_MOVE", "------------------");
            if (mFlag == CLICK_ON_LOW) {
                if(e.getX()>mOffsetHigh-mThumbWidth/2){
                    //防止重叠
                    return true;
                }
                if (e.getX() < 0 || e.getX() <= mThumbWidth/2) {
                ...
                if(e.getX()<mOffsetLow+mThumbWidth/2){
                    //防止重叠
                    return true;
                }
